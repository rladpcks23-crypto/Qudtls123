...
const Cheats = {
  ...
  list: {
    ...
    OHDUDE: ['PCJ-600 오토바이 소환', () => { const P = Game.player; const c = new Car('bike', P.px + 3, P.py, 0); Game.cars.push(c); }],
  },
  key(code) {
    if (!code.startsWith('Key')) return;
    this.buf = (this.buf + code.slice(3)).slice(-16);
    for (const name in this.list) if (this.buf.endsWith(name)) { this.apply(name); this.buf = ''; }
  },
  apply(name) {
...
addEventListener('keydown', e => { if (typeof Game !== 'undefined' && Game.state === 'play') Cheats.key(e.code); });
