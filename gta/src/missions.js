...
// 새 기능 모듈의 저장: SaveExt.mods.이름 = { save(), load(데이터 또는 undefined) }
const SaveExt = {
  mods: {},
  save() { const o = {}; for (const k in this.mods) { try { o[k] = this.mods[k].save(); } catch (e) { } } return o; },
  load(o) { for (const k in this.mods) { try { this.mods[k].load(o ? o[k] : undefined); } catch (e) { console.error('SaveExt', k, e); } } },
};
const Save = {
...
const MISSION_DEFS = [
  ...
  }),
  {
    title: '13. 강철 코끼리',
    reward: 15000,
    intro: [['마담 윤', '청룡파 잔당이 무기를 사 모은다는 소문이야. 우리도 한 수 위를 보여줘야지.'], ['마담 윤', '바다 건너 포트 네온 기지(군사 섬)에 라이노 전차가 있어. 둑길로 들어가. 끌고 와. 차고에 넣으면 경찰 쪽은 우리가 정리해 줄게.'], ['도움말', '전차는 느리지만 차를 밀어 부순다. 발사 버튼 = 주포. 기지에 들어가면 수배 ★4가 붙는다.']],
  ...
  },
  {
    title: '14. 하늘의 주인',
    reward: 18000,
    intro: [['조니 박', '큰일이야. 기지 녀석들이 우리를 노리고 공격 헬기 세 대를 띄운대.'], ['조니 박', '포트 네온 기지(군사 섬)에서 헌터 헬기를 가져와서 먼저 떨어뜨려. 미사일은 적 헬기 쪽을 겨누고 쏘면 공중으로 날아간다.'], ['도움말', '헬기: 가속 = 이륙·전진, 핸들 = 방향. 발사 = 기관포, 드리프트/스페이스 = 미사일. 비행 중 하차 두 번 = 낙하산.']],
