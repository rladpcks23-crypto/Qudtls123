...
function genWorld(seed) {
  ...
  W.curBlock = null; W.guardRoads = false; W.planRoad = new Uint8Array(MW * MH); W.alleyT = new Uint8Array(MW * MH); W.planSides = [];
  W.blocks = []; W.buildings = []; W.trees = []; W.lamps = []; W.parking = []; W.places = {};
  // W.guardRoads 동안에는 미리 계획한 길(W.planRoad) 위를 다른 것으로 덮지 않는다 → 길을 먼저 깔고 그 옆에 건물을 앉힌다
  const set = (x, y, t) => { if (x >= 0 && y >= 0 && x < MW && y < MH) { const i = x + y * MW; if (W.guardRoads && (W.planRoad[i] || W.alleyT[i]) && t !== TL.ROAD) return; if (W.curBlock && W.region[i] >= 0 && W.region[i] !== W.curBlock.id) return; W.tiles[i] = t; } };
  ...
  function planRoads() {
    ...
    const A = W.airport;
    const mark = (x, y) => { if (x < 2 || y < 2 || x >= MW - 2 || y >= MH - 2) return; const i = tIdx(x, y), t = W.tiles[i]; if (t === TL.ROAD || t === TL.WATER) return; if (A && A.shape(x, y)) return; if (Math.hypot(x + 0.5 - cx, y + 0.5 - cy) < R0) return; M[i] = 1; };
    const side = (x, y, tree) => W.planSides.push([x, y, tree]);
    // 에투알 둘레
    const Lmax = 0.1 * MW, rr0 = 0.07 * MW;
    for (let y = 1; y < MH - 1; y++) for (let x = 1; x < MW - 1; x++) {
      const dx = x + 0.5 - cx, dy = y + 0.5 - cy, d = Math.hypot(dx, dy);
      if (d > Lmax + 2) continue;
      let hit = Math.abs(d - rr0) < 1.4;
      if (!hit && d > R0 && d < Lmax) for (let k = 0; k < 8; k++) { const a = Math.PI / 8 + k * Math.PI / 4, ux = Math.cos(a), uy = Math.sin(a); if (dx * ux + dy * uy > 0 && Math.abs(dx * uy - dy * ux) < 1.6) { hit = true; break; } }
      if (hit) mark(x, y);
    }
    for (let k = 0; k < 8; k++) { const a = Math.PI / 8 + k * Math.PI / 4, ux = Math.cos(a), uy = Math.sin(a); for (let s = R0 + 2, n = 0; s < Lmax; s += 2.6, n++) for (const sg of [-1, 1]) side(cx + ux * s - uy * sg * 2.4, cy + uy * s + ux * sg * 2.4, n % 4 !== 0); }
    for (let a = 0, n = 0; a < TAU; a += 2.6 / rr0, n++) for (const sg of [-1, 1]) side(cx + Math.cos(a) * (rr0 + sg * 2.4), cy + Math.sin(a) * (rr0 + sg * 2.4), n % 4 !== 0);
    // 옛길
    const hoodC = HOODS.map(h => ({ x: h.u * MW, y: h.v * MH }));
    const pairs = new Set();
    ...
    const landT = (x, y) => get(Math.floor(x), Math.floor(y)) !== TL.WATER;
    const trace = (ax, ay, bx, by, wid, seed) => {
      if (!landT(ax, ay) || !landT(bx, by)) return;
      ...
      for (let k = 0; k < 3000; k++) {
        ...
        h += clamp(angNorm(want - h), -0.045, 0.045);
        let nx = x + Math.cos(h) * 0.7, ny = y + Math.sin(h) * 0.7;
        if (!landT(nx + Math.cos(h) * 3, ny + Math.sin(h) * 3)) {
        ...
        x = nx; y = ny;
        for (let oy = -wid; oy <= wid; oy++) for (let ox = -wid; ox <= wid; ox++) if (ox * ox + oy * oy <= wid * wid + 0.5) mark(Math.floor(x) + ox, Math.floor(y) + oy);
        if (k % 9 === 0) side(x - Math.sin(h) * (wid + 2.2), y + Math.cos(h) * (wid + 2.2), k % 27 !== 0);
    ...
    // 해안도로: 바다(강 제외)에서 일정 거리 (chamfer 거리 변환 → 모서리가 둥글다)
    const riv = new Uint8Array(MW * MH);
    for (let i = 0; i < NX - 1; i++) for (let j = 0; j < NY - 1; j++) if (W.cellRiver[i][j]) for (let y = W.HY[j]; y < W.HY[j + 1] + 2; y++) for (let x = W.VX[i]; x < W.VX[i + 1] + 2; x++) riv[tIdx(x, y)] = 1;
    const dW = new Float32Array(MW * MH).fill(1e9);
    for (let i = 0; i < MW * MH; i++) if (W.tiles[i] === TL.WATER && !riv[i]) dW[i] = 0;
    const pass = fwd => { const s = fwd ? 1 : -1; for (let y = fwd ? 1 : MH - 2; fwd ? y < MH - 1 : y > 0; y += s) for (let x = fwd ? 1 : MW - 2; fwd ? x < MW - 1 : x > 0; x += s) { const i = tIdx(x, y); dW[i] = Math.min(dW[i], dW[i - s] + 3, dW[i - s * MW] + 3, dW[i - s * MW - s] + 4, dW[i - s * MW + s] + 4); } };
    pass(true); pass(false);
    const Rc = 16 * 3;
    for (let y = 2; y < MH - 2; y++) for (let x = 2; x < MW - 2; x++) { const d = dW[tIdx(x, y)]; if (d >= Rc - 3 && d < Rc + 3 && hash2(x >> 5, y >> 5) < 0.75) mark(x, y); }
  }
  // ---------- 계획한 길을 포장한다 (건물을 다 앉힌 뒤): 아스팔트 + 양옆 인도 + 가로수·가로등 ----------
  function paveRoads() {
    const M = W.planRoad;
    for (let i = 0; i < MW * MH; i++) if (M[i] && W.tiles[i] !== TL.BUILD && W.tiles[i] !== TL.WATER) { W.tiles[i] = TL.ROAD; W.roadK[i] = 4; } else if (M[i]) M[i] = 0;
    for (let y = 2; y < MH - 2; y++) for (let x = 2; x < MW - 2; x++) {
      const i = tIdx(x, y); if (!M[i]) continue;
      for (const [dx, dy] of DIRS) { const j = tIdx(x + dx, y + dy), t = W.tiles[j]; if (!M[j] && (t === TL.GRASS || t === TL.PLAZA || t === TL.SAND || t === TL.LOT) && W.dist[j] !== DIST.BASE && W.dist[j] !== DIST.AIRPORT) W.tiles[j] = TL.WALK; }
    }
    for (const [x, y, tree] of W.planSides) { const tx = Math.floor(x), ty = Math.floor(y); if (tx < 1 || ty < 1 || tx >= MW - 1 || ty >= MH - 1) continue; const t = W.tiles[tIdx(tx, ty)]; if (t === TL.ROAD || t === TL.WATER || t === TL.BUILD || M[tIdx(tx, ty)]) continue; if (tree) addTree(x * T, y * T, 'tree'); else W.lamps.push({ x: x * T, y: y * T }); }
    W.parking = W.parking.filter(p => W.tiles[tIdx(clamp(Math.floor(p.x / T), 0, MW - 1), clamp(Math.floor(p.y / T), 0, MH - 1))] === TL.LOT);
    for (const b of W.blocks) for (const L of b.lots || []) if (L.b && L.b.removed) { L.b = null; L.used = true; }
    const bm = new Uint8Array(MW * MH); for (let i = 0; i < MW * MH; i++) bm[i] = M[i] ? 1 : W.alleyT && W.alleyT[i] ? 2 : 0;
    W.boulevardMask = bm;
  }
  // 블록 하나를 통째로 비우고 랜드마크를 세운다
  function landmarkBlock(u, v, pred, build) {
    const hasRoad = b => { for (let y = b.y0; y <= b.y1; y += 1) for (let x = b.x0; x <= b.x1; x += 1) if (W.planRoad[tIdx(x, y)] || (W.alleyT && W.alleyT[tIdx(x, y)])) return true; return false; };
    let cands = W.blocks.filter(b => !b.etoile && !b.landmark && pred(b) && b.x1 - b.x0 >= 9 && b.y1 - b.y0 >= 9);
    const clean = cands.filter(b => !hasRoad(b)); if (clean.length) cands = clean;
  ...
  { const _t = performance.now(); planRoads(); W.genT.planRoads = Math.round(performance.now() - _t); }
  W.guardRoads = true;
  ...
  const addTree = (x, y, kind) => (W.guardRoads && W.planRoad[tIdx(clamp(Math.floor(x / T), 0, MW - 1), clamp(Math.floor(y / T), 0, MH - 1))]) ? 0 : W.trees.push({ x, y, kind, r: kind === 'palm' ? 2.2 : rr(1.9, 2.8), h: kind === 'palm' ? rr(7, 9) : rr(5, 8), hue: R() });
  const addBuilding = (x0, y0, x1, y1, h, kind, color) => {
    if (W.guardRoads) { // 계획된 길과 겹치면 길 쪽 변을 깎아 낸다 (다 깎이면 짓지 않는다)
      for (let k = 0; k < 16; k++) {
        let hit = null; for (let y = y0; y <= y1 && !hit; y++) for (let x = x0; x <= x1; x++) if (W.planRoad[tIdx(x, y)] || W.alleyT[tIdx(x, y)] || (W.curBlock && W.region[tIdx(x, y)] !== W.curBlock.id)) { hit = [x, y]; break; }
        if (!hit) break;
        const [rx, ry] = hit, d = [rx - x0, x1 - rx, ry - y0, y1 - ry], m = Math.min(...d);
        if (m === d[0]) x0 = rx + 1; else if (m === d[1]) x1 = rx - 1; else if (m === d[2]) y0 = ry + 1; else y1 = ry - 1;
        if (x0 > x1 || y0 > y1) break;
      }
      let bad = x0 > x1 || y0 > y1; if (!bad) for (let y = y0; y <= y1 && !bad; y++) for (let x = x0; x <= x1; x++) if (W.planRoad[tIdx(x, y)] || W.alleyT[tIdx(x, y)] || (W.curBlock && W.region[tIdx(x, y)] !== W.curBlock.id)) { bad = true; break; }
      if (bad) return { x0, y0, x1, y1, h, kind, color, removed: true }; // 짓지 않음 (나중에 필지를 비운다)
    }
  ...
  const addParkingRow = (x0, y0, x1, y1) => {
    ...
    for (let x = x0 * T + 1.6; x + 1.6 <= (x1 + 1) * T; x += 3.2) {
      const pr = (px, py) => W.planRoad[tIdx(clamp(Math.floor(px / T), 0, MW - 1), clamp(Math.floor(py / T), 0, MH - 1))];
      if (!pr(x, y0 * T + T * 0.5)) W.parking.push({ x, y: y0 * T + T * 0.5 + 0.6, a: -Math.PI / 2, car: null, cd: 0 });
      if (y1 - y0 >= 2 && !pr(x, (y1 + 1) * T - T * 0.5)) W.parking.push({ x, y: (y1 + 1) * T - T * 0.5 - 0.6, a: Math.PI / 2, car: null, cd: 0 });
    }
  };
  ...
  for (const b of W.blocks) {
    ...
    const lots = [];
    // 골목: 큰 블록은 가운데 좁은 길로 먼저 나누고, 나뉜 두 쪽에 필지를 만든다 (골목이 건물·주차장을 가르지 않는다)
    const bw = ix1 - ix0 + 1, bh = iy1 - iy0 + 1;
    const ax = b.district !== DIST.DOWNTOWN && bw >= 13 && R() < 0.8 ? Math.floor((ix0 + ix1) / 2) : -1;
    const ay = b.district !== DIST.DOWNTOWN && bh >= 13 && R() < 0.5 ? Math.floor((iy0 + iy1) / 2) : -1;
    const xs = ax >= 0 ? [[ix0, ax - 1], [ax + 1, ix1]] : [[ix0, ix1]], ys = ay >= 0 ? [[iy0, ay - 1], [ay + 1, iy1]] : [[iy0, iy1]];
    for (const [x0, x1] of xs) for (const [y0, y1] of ys) subdivide(x0, y0, x1, y1, lotSize[0], lotSize[1], lots);
    const alley = (x, y) => { const i = tIdx(x, y); if (W.tiles[i] === TL.ROAD || W.planRoad[i] || W.region[i] !== b.id || W.tiles[i] === TL.BUILD) return; set(x, y, TL.ROAD); W.roadK[i] = 5; W.alleyT[i] = 1; };
    if (ax >= 0) for (let y = b.y0 + 1; y < b.y1; y++) alley(ax, y);
    if (ay >= 0) for (let x = b.x0 + 1; x < b.x1; x++) alley(x, ay);
    const onRoadSide = L => { for (let x = L.x0 - 1; x <= L.x1 + 1; x++) for (const y of [L.y0 - 1, L.y1 + 1]) { const i = tIdx(x, y); if (W.planRoad[i] || W.roadK[i] >= 6) return true; } for (let y = L.y0; y <= L.y1; y++) for (const x of [L.x0 - 1, L.x1 + 1]) { const i = tIdx(x, y); if (W.planRoad[i] || W.roadK[i] >= 6) return true; } return false; };
    for (const L of lots) {
      ...
      } else if (b.district === DIST.RESID && (R() < 0.55 || onRoadSide(L))) { // 큰길·고속도로 옆은 가게 (자리에 맞는 건물) // 주택가 속 동네 가게(상가)
  ...
  W.curBlock = null;
  // 길 때문에 짓지 못한 필지는 비운다 (장소가 없는 건물을 고르지 않게)
  for (const b of W.blocks) for (const L of b.lots || []) if (L.b && L.b.removed) { L.b = null; L.used = true; }
