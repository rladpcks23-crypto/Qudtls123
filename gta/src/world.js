...
function genWorld(seed) {
  ...
  function reserveAirport() {
    ...
    W.airport = { cx, cy, rx, ry, shape, inner, bx0: cx - rx - 8, bx1: cx + rx + 8, by0: cy - ry - 8, by1: cy + ry + 8 };
  }
  function buildAirportIsland() {
    ...
    for (const s of [-1, 1]) addBuilding(s < 0 ? mx - tw - 14 : mx + tw + 1, my - 1, s < 0 ? mx - tw - 1 : mx + tw + 14, my + 1, 10, 'terminal', '#b8c4cf');
    const cA = my - th - Math.max(5, Math.round((ary - th) / 2)), cB = my + th + Math.max(5, Math.round((ary - th) / 2));
    for (const c of [cA, cB]) { addBuilding(mx - Math.round(tw * 1.2), c - 1, mx + Math.round(tw * 1.2), c + 1, 10, 'terminal', '#b8c4cf'); addBuilding(mx - 1, Math.min(c, my) + 2, mx + 1, Math.max(c, my) - 2, 7, 'terminal', '#aab6c1'); }
    const ct = addBuilding(mx + arx + 4, my - 1, mx + arx + 5, my, 40, 'ctower', '#9aa3ad'); ct.label = '관제탑';
    W.decos.push({ t: 'cab', x: (mx + arx + 5) * T, y: (my + 0.5) * T, z: 40, c: '#2b4a5a' });
    addBuilding(mx + arx + 9, my - 8, mx + arx + 18, my - 2, 12, 'warehouse', '#9aa3ad').label = '화물 청사';
    pv(mx + arx + 8, my - 9, mx + arx + 19, my + 6, TL.LOT);
    for (let k = 0; k < 3; k++) { const hx = mx - arx - 14 - k * 11; pv(hx - 1, my - 5, hx + 9, my + 5, TL.LOT); addBuilding(hx, my - 3, hx + 7, my + 3, 10, 'hangar', '#7a8290'); W.decos.push({ t: 'vault', x0: hx * T, y0: (my - 3) * T, x1: (hx + 8) * T, y1: (my + 4) * T, z: 10, c: '#6b7480' }); }
    // 여객기 게이트
    A.gates = [];
...
function buildMinimap() {
  ...
  for (let i = 0; i < MW * MH; i++) {
    ...
    if (bm && bm[i] === 1 && World.tiles[i] === TL.ROAD) col = '#e8d59c'; // 방사형 대로 · 그랑 불바르
    else if (bm && bm[i] === 2) col = '#9ea3ab'; // 골목
    else if (World.tiles[i] === TL.ROAD && World.roadK[i] >= 6) col = '#f6d365'; // 고속도로
    else if (World.tiles[i] === TL.ROAD && (World.hwLine && World.hwLine[i])) col = '#f6d365';
